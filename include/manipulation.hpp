...
Eigen::MatrixXd form_closure(Eigen::VectorXd f, Eigen::MatrixXd A,
                             Eigen::VectorXd b, Eigen::MatrixXd F,
                             Eigen::VectorXd Aeq, Eigen::MatrixXd beq) {
  if (A.cols<A.cols) {
    throw NotFormClosure();
  }
  //TODO (fix)more neat rank deficiency but reduandant constraints, you can use eign api.
  //
